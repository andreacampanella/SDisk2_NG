...
void set_contrast()
{
	...
	if (id_of_config_file != -1)
	{
		...
		if (FAT_partitionType == PARTITION_TYPE_FAT32)
			cluster = (unsigned long)config_file->firstClusterHI << 16 | (unsigned long)config_file->firstClusterLO;
		else
			cluster = (unsigned long)config_file->firstClusterLO;
	...
	do
	{
		...
		if (file)
		{
			...
			if (strncmp_P((char *)file->name, CFG, 11) == 0) // found config file
			{
				...
				if (FAT_partitionType == PARTITION_TYPE_FAT32)
					cluster = (unsigned long)file->firstClusterHI << 16 | (unsigned long)file->firstClusterLO;
				else
					cluster = (unsigned long)file->firstClusterLO;
				SD_readSingleBlock(getSector(cluster));
				struct Sdisk_config_structure *config = (struct Sdisk_config_structure *)buffer;
				if (config->checksum == CHECKSUM_CONFIG)
				{
...
unsigned char is_a_nic(struct dir_Structure *file)
{
	if (!file)
		return 0;
	if (file->name[0] == 0x00 || file->name[0] == 0xE5 || file->name[0] == 0x5F)
		return 0; // deleted or empty file
	if (file->name[8] == 'N' && file->name[9] == 'I' && file->name[10] == 'C')
		return 1;
	return 0;
}
